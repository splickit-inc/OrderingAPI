...
class Php5Redis {
	...
	function __construct($host='tweb01.splickit.com', $port = 6379) {
		$this->host = $host;
		$this->port = $port;
	}
	...
	private function debug($msg){
		...
		if ($this->debug) myerror_log(sprintf("[Php5Redis] %s\n", $msg));
	}
	...
	function publish2($key, $value)
	{
		return $this->cmd ( "PUBLISH $key \"".strlen($value)."\r\n".$value."\"");
	}
	
	function publish($key, $value) {
		//$value = str_replace("\n", "", $value);
		$value = urlencode($value);
		$result = $this->cmd2( "PUBLISH $key ".$value."");
		myerror_log("Redis result is: ".$result);		
		return $result;
	}
	
	private function cmd2($command) {
		$this->debug('Command: '.$command);
		$this->connect ();
		$s = $command. "\r\n";
		myerror_log("the command: x".$s."x");
		while ( $s ) {
			$i = fwrite ( $this->_sock, $s );
			if ($i == 0)
				break;
