<?php
class LevelupbroadcastPaymentService extends SplickitPaymentService
{
    var $successfull_process_payment_message = "Broadcast Payment Passed Through";
...
//        $ending_balance_after = $balance_after_order + $this->amount;
        if ($bc_resource = $balance_change_adapter->addRow ($this->billing_user_id, 0.00, 0.00, 0.00, 'LevelUpBroadcast',$this->name, $order_resource->order_id, 'none','Level Up Broadcast Payment')) {
            ...
            MailIt::sendErrorEMail('Error thrown in PlaceOrderController','ERROR*****************  FAILED TO ADD ROW TO BALANCE CHANGE TABLE, user_id = '.$order_resource->user_id.', in LeveUPBroadcastPaymentService: '.$balance_change_adapter->getLastErrorText());
        }
