...
class StarmicrosPrinterControllerTest extends PHPUnit_Framework_TestCase
{
	var $stamp;
	var $ids;
...
//    public function testPullNoMessage()
//    {
//
//    }
    ...
    function testGetMessage()
    {
        ...
        $message_resource = MerchantMessageHistoryAdapter::getMessageByOrderIdAndFormat($order_id,'RUC');
        $message_resource->next_message_dt_tm = time() - 10;
        $message_resource->save();

        $url = "/messagemanager/getnextmessagebymerchantid/$merchant_numeric_id/starmicros/message.txt";
        ...
        $request = createRequestObject($url,'POST',$body);
        $starmicros_printer_controller = ControllerFactory::generateFromUrl($url,$m,$u,$request);
        $this->assertEquals('StarmicrosPrinterController',get_class($starmicros_printer_controller));
        $response = $starmicros_printer_controller->pullNextMessageResourceByMerchant($merchant_numeric_id);
        $message_text = $response->message_text;
        $expected_post_response = '{"jobReady": true,"mediaTypes": ["text/plain"]}';
        $this->assertEquals($expected_post_response,$message_text);

        $url = "/app2/messagemanager/getnextmessagebymerchantid/$merchant_numeric_id/starmicros/message.txt?mac=00:11:62:0d:77:9f&type=text/plain";
        $request = createRequestObject($url,'GET');

        $starmicros_printer_controller = ControllerFactory::generateFromUrl($url,$m,$u,$request);
        $this->assertEquals('StarmicrosPrinterController',get_class($starmicros_printer_controller));
        $message_resource = $starmicros_printer_controller->pullNextMessageResourceByMerchant($merchant_numeric_id);
        $message_text = $message_resource->message_text;
        myerror_log($message_text);
        $this->assertContains("Order Id:  $order_id",$message_text);
    }
...
//        $sql = "INSERT INTO `Lookup` VALUES(null, 'message_template', 'RUC', '/order_templates/fax/epson_item_all.txt', 'Y', now(), now(), 'N');";
...
if (isset($_SERVER['XDEBUG_CONFIG']) && !defined('PHPUnit_MAIN_METHOD')) {
	StarmicrosPrinterControllerTest::main();
}
