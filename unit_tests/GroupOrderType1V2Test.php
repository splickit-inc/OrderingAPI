...
class GroupOrderType1V2Test extends PHPUnit_Framework_TestCase
{
    var $stamp;
    var $user;
    var $merchant_id;
    ...
    function createGroupOrder($user,$note,$emails)
    {
        $request = new Request();
        $request->data = array("merchant_id" => $this->merchant_id, "note" => $note, "merchant_menu_type" => 'Pickup', "participant_emails" => $emails, "group_order_type" => 1);
    ...
    function testGuestUserCannotCreateGroupOrder()
    {
    ...
    {
        $group_order_resource = Resource::dummyfactory(array("sent_ts" => '2014-11-11 00:00:00', "expires_at" => time() + 100, "status" => 'submitted'));
    ...
    function testCreateAdminEmail()
    {
        ...
        $response = $group_order_controller->createGroupOrder();

        $records = $mmha->getRecords(array("info" => "subject=Your PitaPit Online Group Order Has Been Started!;from=PitaPit Group Ordering;", "message_delivery_addr" => $user['email'], "order_id" => $response->order_id));
    ...
    function testCreateGroupEmails()
    {
        ...
        $data['group_order_token'] = $token;
        $data['ucid'] = $token;
        $data['user_id'] = $user['user_id'];
        $data['merchant_id'] = $this->merchant_id;
        $cart_resource = CartsAdapter::createCart($data);

        $group_order_controller->sendGroupOrderInfoAsEmails(array('group_order_token' => $token, 'participant_emails' => $emails, 'merchant_id' => $merchant_id, 'merchant_menu_type' => $merchant_menu_type, 'order_id' => $cart_resource->order_id));
        $mmha = new MerchantMessageHistoryAdapter($mimetypes);
        $records = $mmha->getRecords(array("info" => "subject=Invitation To A " . ucwords(getSkinNameForContext()) . " Group Order;from=" . ucwords(getSkinNameForContext()) . " Group Ordering;", "order_id" => $cart_resource->order_id));
    ...
    function testCreateGroupOrderWithDeliveryLocation()
    {
        ...
        $request = new Request();
        $request->data = array("merchant_id" => $this->merchant_id, "note" => $note, "merchant_menu_type" => 'Delivery', "participant_emails" => $emails, "user_addr_id" => $user_address_id, "group_order_type" => 1);
        $request->url = "app2/apiv2/grouporders";
        $request->method = "POST";
        $group_order_controller = new GroupOrderController($mt, $user, $request, 5);
        $resource = $group_order_controller->processV2Request();
     ...
     */
    function testAddToGroupOrderDelivery($group_order_token)
    {
        //setContext("com.splickit.snarfs");
        ...
        $request = createRequestObject('/app2/apiv2/cart','POST',$json_encoded_data);
        ...
        $participant_ucid = $cart_resource->ucid;
        // now checkout

        $request = createRequestObject("/app2/apiv2/cart/$participant_ucid/checkout",'GET');
        $place_order_controller = new PlaceOrderController($mt, $user, $request);
        ...
        $this->assertNotNull($ucid,"It should have the ucid of the child order");
        $this->assertNotNull($checkout_resource->cart_ucid,"It should have backwards compatible ucid of the child order ");

        // validate that order was added to the mapping table
        $cart_order_resource = Resource::find(new CartsAdapter($m),"$ucid");
        ...
        $this->assertEquals("In Process", $record['status']);

        // validate note was added
        $this->assertEquals($cart_note, $cart_order_resource->note, "note should ahve been set on the cart order record");

        // make sure there is no lead time array, only the single choice of submit
        $this->assertTrue($checkout_resource->show_submit_to_parent_group_order_content, "Flag to submit to group order should have been set");
        $this->assertCount(0, $checkout_resource->lead_times_array);
        $this->assertCount(0, $checkout_resource->accepted_payment_types);
        $this->assertCount(0, $checkout_resource->tip_array);

        // do not allow promos for type1 grouporders

        $request = createRequestObject("/app2/apiv2/cart/$ucid/checkout",'POST',json_encode(array("promo_code" => "sumdumpromocode")));
        $place_order_controller = new PlaceOrderController($mt, $user, $request);
        $promo_resource_result = $place_order_controller->processV2Request();
        $this->assertNotNull($promo_resource_result->error);
        $this->assertEquals("Sorry, promos are not enabled on this type of group order.", $promo_resource_result->error);

        // now place the order which should add it to the group order
        $place_order_result_resource = $this->placeOrderToBePartOfGroupOrder($user, $this->ids['merchant_id'], $checkout_resource->ucid, $checkout_resource, $time);
        $this->assertNull($place_order_result_resource->error);

        // map should have been set to subitted
        $record = $gorma->getRecord(array("user_order_id" => $cart_order_resource->order_id));
        $this->assertEquals("Submitted", $record['status']);

        $this->assertEquals("Your order has been added to the group order.", $place_order_result_resource->user_message);

        $god_record = getStaticRecord(array("user_id" => $user['user_id'], "group_order_id" => $group_order_id), 'GroupOrderDetailAdapter');
        $this->assertNotNull($god_record, "It should have recorded a group order detail record");
        $god_resource = Resource::dummyfactory($god_record);
        ...
        $this->assertContains("Tim Z. - $cart_note", $group_order_detail_record['order_json']);

        // now verify that the cart is set to G
        $old_cart_record = CartsAdapter::staticGetRecordByPrimaryKey($ucid, 'Carts');
    ...
    function testCreateGroupOrder()
    {
        ...
        $g_data = array("merchant_id" => $this->merchant_id, "note" => $note, "merchant_menu_type" => 'Delivery', "participant_emails" => $emails, "user_addr_id" => $user_address_id, "group_order_type" => 1);
        $request = createRequestObject("app2/apiv2/grouporders", "POST", json_encode($g_data));
        $group_order_controller = new GroupOrderController($mt, $user, $request, 5);
        $resource = $group_order_controller->processV2Request();
    ...
    function testValidateEmail($group_order_token)
    {
        ...
        $records = $mmha->getRecords(array("info" => "subject=Invitation To A Snarfs Group Order;from=Snarfs Group Ordering;", "order_id" => $cart['order_id']));
     ...
     * @depends testCreateGroupOrder
     */
    function testAddToGroupOrderUsingCartForAdminUser($group_order_token)
    {
        setContext("com.splickit.snarfs");
        ...
        $admin_name_string = $user['first_name'] . ' ' . $user['last_name'][0] . '.';

        $order_data = OrderAdapter::getSimpleCartArrayByMerchantId($this->ids['merchant_id']);
        $cart_note = "Admin cart note";
        $order_data['note'] = $cart_note;
        $order_data['group_order_token'] = $group_order_token;
        ...
        $this->assertNotEquals($group_order_token, $cart_resource->ucid, "It should have a diffenrt cart id then the group order since the admin is adding, not checking out the group order");
        ...
        // get checkout data to make sure there is no lead time array, only the single choice of submit
        $order_data['user_addr_id'] = null;
        $order_data["submitted_order_type"] = 'Delivery';
        $checkout_resource = $this->getCheckoutForGroupOrder($user, $cart_resource->ucid, $time, $order_data);
        $this->assertNull($checkout_resource->error, "It should not have had a checkout error");
        $this->assertCount(0, $checkout_resource->lead_times_array);
        $this->assertCount(0, $checkout_resource->accepted_payment_types);
        $this->assertCount(0, $checkout_resource->tip_array);
        $this->assertTrue($checkout_resource->show_submit_to_parent_group_order_content, "Flag to submit to group order should have been set");

        $cart = CompleteOrder::getItemsForGroupOrderFromCartId($cart_resource->ucid);
        // now place the order which should add it to the group order
        $place_order_result_resource = $this->placeOrderToBePartOfGroupOrder($user, $this->ids['merchant_id'], $cart_resource->ucid, $checkout_resource, $time);
        $this->assertNull($place_order_result_resource->error);
        ...
        $this->assertTrue($god_resource->group_order_detail_id > 100);
        $group_order_detail_record = GroupOrderDetailAdapter::staticGetRecordByPrimaryKey($god_resource->group_order_detail_id, 'GroupOrderDetailAdapter');
        $this->assertContains("$admin_name_string - $cart_note", $group_order_detail_record['order_json']);
     ...
     * @depends testCreateGroupOrder
     */
    function testAddToGroupOrderUsingCartThirdUserMakeSureSummaryIsOnlyThisOrder($group_order_token)
    {
        setContext("com.splickit.snarfs");
        $group_order_record = getStaticRecord(array("group_order_token" => $group_order_token), 'GroupOrderAdapter');
        $group_order_id = $group_order_record['group_order_id'];
        $user_resource = createNewUser();
        $user_resource->first_name = 'Bob';
        $user_resource->last_name = 'Newheart';
        $user_resource->save();
        $user = logTestUserResourceIn($user_resource);
        ...
        // get checkout data to make sure there is no lead time array, only the single choice of submit
        $checkout_resource = $this->getCheckoutForGroupOrder($user, $cart_resource->ucid, $time);
        $checkout_summary = $checkout_resource->order_summary;
        $this->assertTrue($checkout_resource->show_submit_to_parent_group_order_content, "Flag to submit to group order should have been set");
        $this->assertCount(0, $checkout_resource->lead_times_array);
        $this->assertCount(0, $checkout_resource->accepted_payment_types);
        $this->assertCount(0, $checkout_resource->tip_array);

        // now place the order which should add it to the group order and make sure the summary is only for this participant
        $place_order_result_resource = $this->placeOrderToBePartOfGroupOrder($user, $this->ids['merchant_id'], $cart_resource->ucid, $checkout_resource, $time);
        $this->assertNull($place_order_result_resource->error);
        $place_order_summary = $place_order_result_resource->order_summary;
        $this->assertCount(1, $place_order_summary['cart_items'], "There should only be the one item in teh cart");
        $this->assertEquals($checkout_summary['receipt_items'], $place_order_summary['receipt_items'], "Place order summary should be the same as the checkout order summary");
    ...
    function testGetGroupOrderForSubmit($group_order_token)
    {
        ...
        $this->assertCount(4, $go_resource->order_summary['cart_items'], "there should be four items in the group order");
        $this->assertEquals(3, $go_resource->total_submitted_orders);
    }

    /**
     * @depends testCreateGroupOrder
     */
    function testCheckoutType1GroupOrder($group_order_token)
    {
        setContext("com.splickit.snarfs");
        $group_order_record = GroupOrderAdapter::staticGetRecord(array("group_order_token" => $group_order_token), 'GroupOrderAdapter');
        $user = logTestUserIn($group_order_record['admin_user_id']);
        $checkout_resource = $this->getCheckoutForGroupOrder($user, $group_order_token, getTomorrowTwelveNoonTimeStampDenver());
        $this->assertTrue(count($checkout_resource->lead_times_array) > 10, "There should pickup times availble");
        $this->assertCount(2, $checkout_resource->accepted_payment_types, "There should be 2 payment types available for checkoiut");
        $this->assertNotNull($checkout_resource->order_summary);
        $this->assertCount(4, $checkout_resource->order_summary['cart_items'], "there should be four item in the group order");
    }
    ...
    function testCreateGroupOrderWithAutoSendTimeSubmittedButDONOTCreateActivity()
    {
        ...
        $order_data['notes'] = "sum dum note";
        $request = createRequestObject("app2/apiv2/grouporders",'POST',json_encode($order_data));
        $group_order_controller = new GroupOrderController($mt, $user, $request, 5);
        ...
        // now check that there shoudl be no activity
        $this->assertEquals(0,$group_order_resource->auto_send_activity_id,"there should not be a group order autosend activity id");

        $activity_history_adapter = new ActivityHistoryAdapter($mimetypes);
        $record = $activity_history_adapter->getRecord(array("info"=>"group_order_id=$group_order_id"));
        $this->assertNull($record,"there should not be a record");

        $carts_adapter = new CartsAdapter($mimetypes);
        $cart = $carts_adapter->getRecordFromPrimaryKey($group_order_token);
        $this->assertEquals(OrderAdapter::ORDER_IS_IN_PROCESS_CART, $cart['status']);

        return $group_order_resource;
    }

    /**
     * @depends testCreateGroupOrderWithAutoSendTimeSubmittedButDONOTCreateActivity
     */
     ...
     * @depends testCreateGroupOrderWithAutoSendTimeSubmittedButDONOTCreateActivity
     */
    function testStartAddToGroupOrderButDontFinish($group_order_resource)
    {
        $group_order_token = $group_order_resource->group_order_token;
        $user_resource = createNewUserWithCCNoCVV();
        $user = logTestUserResourceIn($user_resource);
        ...
        $this->assertNotNull($ucid,"It should have the ucid of the child order");
        $this->assertNotNull($cart_resource->cart_ucid,"It should have backwards compatible ucid of the child order ");
     ...
     */
    function testSendGroupOrder($group_order_token)
    {
        $cart_record = CartsAdapter::staticGetRecordByPrimaryKey($group_order_token, 'Carts');
        $user = logTestUserIn($cart_record['user_id']);
        $user_id = $user['user_id'];

        $order_data['merchant_id'] = $cart_record['merchant_id'];
        $order_data['note'] = "the new cart note";
        $order_data['user_id'] = $user_id;
        $order_data['cart_ucid'] = $cart_record['ucid'];
        $order_data['tip'] = (rand(100, 1000)) / 100;
    ...
    /**
     * @depends testSendGroupOrder
     */
    function testAddInProcessOrderToSubmitteGroupOrder($order_id)
    {
        $base_order = CompleteOrder::getBaseOrderData($order_id);
        $group_order_record = getStaticRecord(array("order_id"=>$base_order['order_id']),'GroupOrderAdapter');
        $goioma = new GroupOrderIndividualOrderMapsAdapter();
        $mapping_records = $goioma->getRecords(array("group_order_id"=>$group_order_record['group_order_id']));
        $mapping_record = $mapping_records[0];
        $user_order_id = $mapping_record['user_order_id'];
        $user_base_order = CompleteOrder::getBaseOrderData($user_order_id);
        $ucid = $user_base_order['ucid'];
        $user = logTestUserIn($user_base_order['user_id']);
        $request = createRequestObject("/apiv2/cart/$ucid/checkout",'GET');
        $place_order_conroller = new PlaceOrderController($m,$user,$request);
        $checkout_resource = $place_order_conroller->processV2Request();
        $order_resource = placeOrderFromCheckoutResource($checkout_resource,$user,$mid,0.00,$time);

        $base_order_of_group_order_after_last_order = CompleteOrder::getBaseOrderData($order_id);
        $this->assertEquals($base_order,$base_order_of_group_order_after_last_order);
...
//  	$this->assertCount(2, $order_summary);
...
//  	return $group_order_token;
    ...
    function getCheckoutForGroupOrder($user, $ucid, $time, $data = null)
    {
        if ($time == null) {
            $time = time();
        }

        $url = "http://localhost/app2/apiv2/cart/$ucid/checkout";
        if ($data) {
    ...
    function placeOrderToBePartOfGroupOrder($user, $merchant_id, $ucid, $checkout_data_resource, $time)
    {
        $order_data = array();
        $order_data['merchant_id'] = $merchant_id;
        ...
        //$order_data['tip'] = (rand(100, 1000)) / 100;
        if ($payment_array = $checkout_data_resource->accepted_payment_types) {
            $order_data['merchant_payment_type_map_id'] = $payment_array[0]['id'];
        }
        if ($lead_times_array = $checkout_data_resource->lead_times_array) {
            $order_data['actual_pickup_time'] = $lead_times_array[0];
        }

        $json_encoded_data = json_encode($order_data);
        $request = createRequestObject("/apiv2/orders/$ucid",'POST',$json_encoded_data);
        $place_order_controller = new PlaceOrderController($mt, $user, $request);
        $place_order_controller->setCurrentTime($time);
