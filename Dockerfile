...
# Copy the conf file
COPY build_files/smaw_vhost.conf /etc/apache2/sites-available/smaw.conf
COPY build_files/smaw_php.ini /etc/php/7.0/apache2/conf.d/21-smawdev.ini
COPY build_files/smaw_php.ini /etc/php/7.0/cli/conf.d/21-smawdev.ini
...
COPY build_files/apache2-foreground.sh /usr/local/bin/

COPY . .
ENV SMAW_ENV production
RUN composer install
RUN ln -sfT /dev/stderr /var/www/smaw/logs/php_errors.log
